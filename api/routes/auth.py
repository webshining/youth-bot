...
from data.config import DIR, FRONTEND_URL, REFRESH_TOKEN_EXPIRE_MINUTES
from database.models import User
from loader import bot

from ..exceptions import tokennotprovided, unauthorized
from ..services import generate_tokens, get_current_user, is_telegram
...
def delete_cookie(response: Response):
    response.delete_cookie(
        key="refreshToken", httponly=True, samesite="none", secure=True
    )
...
async def _auth_redirect(id: int, request: Request, state: str = FRONTEND_URL):
    if is_telegram(dict(request.query_params)):
        ...
        data = {"user": user.model_dump_json(), "accessToken": access_token}
        response = (
            RedirectResponse(f"{state}#{json.dumps(data)}")
            if state
        ...
        return (
            RedirectResponse(f"{state}#{data}")
            if state
            else {"error": unauthorized.detail}
        )
...
async def _refresh(request: Request):
    ...
    response = JSONResponse(
        {"user": user.model_dump(mode="json"), "accessToken": access_token}
    )
    set_cookie(response, refresh_token)
    return response
