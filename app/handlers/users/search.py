        ...
        if song:
            if song.text:
                await call.message.edit_text(
                    song.get_text(chords=False),
                    reply_markup=get_song_markup("song", song.id),
                )
            else:
                await call.answer(
                    _("Looks like the song has no lyrics"), show_alert=True
                )
        else:
            await call.message.edit_text(
                _("Looks like the songs are out of memory or you used /cancel")
            )
    except:
        pass
        ...
        if call.data[5:] == "back":
            if songs:
                ...
                for i, s in enumerate(songs):
                    text += f"\n<b>{i + 1}.</b> <u>{s.name}</u> - {s.artist}"
                await call.message.edit_text(
                    text, reply_markup=get_songs_markup("search", songs)
                )
            else:
                await call.message.edit_text(
                    _("Looks like the songs are out of memory or you used /cancel"),
            ...
            if song:
                if song.file:
                    ...
                    await call.answer(
                        _("Looks like there’s no music on the resource for this song"),
                        show_alert=True,
                    )
            else:
                await call.message.edit_text(
